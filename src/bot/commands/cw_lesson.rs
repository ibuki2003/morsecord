...
impl crate::bot::Bot {
    pub async fn run_command_lesson_start(&self, ctx: &Context, command: &ApplicationCommandInteraction) -> Result<String, String> {
        let mut min_speed = 15.0;
        let mut max_speed = 30.0;
        let mut min_freq = 400.0;
        let mut max_freq = 1000.0;

        command.data.options.iter().try_fold::<_, _, Result<(), String>>((), |_, x| {
            let v = x.value.as_ref()
                .map_or(Err("value not found".to_string()), |v| v.as_f64().ok_or("value is not f64".to_string()))? as f32;
            match x.name.as_str() {
        ...
        let state = Arc::new(Mutex::new(crate::modes::call_lesson::CallLessonModeState::new(speed_range, freq_range, command.channel_id)));
        crate::modes::call_lesson::start(&ctx, gid, state.clone()).await.map_err(|_| "error occured")?;
        let _ = self.switch_mode(gid.0, BotStateMode::CallsignLesson(state)).await;

        Ok("let's start lesson".to_string())
    }

    pub async fn run_command_lesson_end(&self, _ctx: &Context, command: &ApplicationCommandInteraction) -> Result<String, String> {
        let _ = self.switch_mode(command.guild_id.ok_or("no guild")?.0, BotStateMode::Normal).await;
